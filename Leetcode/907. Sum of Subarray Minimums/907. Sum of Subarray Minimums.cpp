...
namespace My907SumofSubarrayMinimums {
	TEST_CLASS(My907SumofSubarrayMinimums) {
public:

	int sumSubarrayMins12(int* arr, int arrSize) {
		long long sum = 0;

		unsigned int minsSum = 0;
		int* posStart = &arr[0];
		int* posEnd = &arr[arrSize - 1];
		while (posStart <= posEnd) {
			int* pos = posStart;
			posStart++;

			if (minsSum >= UINT32_MAX - (30000 * 30000)) {
				sum += minsSum;
				minsSum = *pos;
			} else {
				minsSum += *pos;
	...
	int sumSubarrayMins11(int* arr, int arrSize) {
		long long sum = 0;
	...
	int sumSubarrayMins10(int* arr, int arrSize) {
		long long sum = 0;
	...
	int sumSubarrayMins9(int* arr, int arrSize) {
		long long sum = 0;
		for (int start = 0; start <= arrSize - 1; start++) {
			int min = arr[start];
			int count = min;
			for (int k = start + 1; k < arrSize; k++) {
				if (min > arr[k]) {
					sum += count;
					min = arr[k];
					count = min;
	...
	int sumSubarrayMins8(int* arr, int arrSize) {
		long long sum = 0;
		int min = arr[0];
		int count = 1;
		int arrMinPos = 0;

		for (int k = 0 + 1; k < arrSize; k++) {
			if (min > arr[k]) {
				sum += min * count;
				min = arr[k];
				count = 1;
		...
		for (int start = 1; start <= arrSize - 1; start++) {
			if (start == arrMinPos) {
				count = arrSize - start;
				sum += arrMin * count;
				continue;
			...
			min = arr[start];
			count = 1;
			int tailOnlyMin = arrMinPos >= start + 1;
			if (tailOnlyMin) {
				for (int k = start + 1; k < arrMinPos; k++) {
					if (min > arr[k]) {
						sum += min * count;
						min = arr[k];
						count = 1;
	...
	int sumSubarrayMins7(int* arr, int arrSize) {
		long long sum = 0;
		for (int start = 0; start <= arrSize - 1; start++) {
			int min = arr[start];
			int count = 1;
	...
	int getLeft(int* arr, int arrSize) {
		int* arrStrt = arr - arrSize;
		int* elem = arr;
		while (--arr >= arrStrt) {
			if (*arr <= *elem) {
				break;
	...
	int getRight(int* arr, int arrSize) {
	...
	int sumSubarrayMins7_0(int* arr, int arrSize) {
		long long sum = 0;
		for (int i = 0; i <= arrSize - 1; i++) {
			int left = getLeft(&arr[i], i);
			int right = getRight(&arr[i], arrSize - i);
	...
	int sumSubarrayMins7_1(int* arr, int arrSize) {
		long long sum = 0;
